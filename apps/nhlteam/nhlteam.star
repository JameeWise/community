...
def main(config):
    ...
    emptyDict = {}
    rep = http.get(
        'https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/teams/' + selectedTeam)
    record = rep.json()["team"]["record"]
    if rep.json()["team"]["record"] == emptyDict:
        record = "0-0-0"
    img = http.get('https://a.espncdn.com/combiner/i?img=/i/teamlogos/nhl/500/' +
                   selectedTeam + '.png').body()
    return render.Root(
        child=render.Box(
            render.Row(
                expanded=True,
